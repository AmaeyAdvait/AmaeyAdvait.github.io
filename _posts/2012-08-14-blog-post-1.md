...
<br>
Ok, I know I just dropped three relatively heavy words with little context, but try to keep them in mind, we'll circle back to them towards the end of this blog series.

As an introspective engineer, I stongly believe in building systems that have a strong foundation, a robust framework and a clear application. NeuroAI is a bridge between mind and machine that can help achieve exactly that. Let me illustrate via an example:-
...
a) *<u>Benchmark</u>: "Consider two people(Alice and Bob) sitting in two different rooms. Both are given k bits of information regarding a particular (static) scene and are asked to visualise the same in their minds. What is the minimum threshold value k beyond which the probability that they visualise different scenes drops below some small {% raw %}$$\epsilon>0$${% endraw %}"*
...
c) *"<u>Improve</u>: Now consider Alice and a neuroAI system named Bob sitting in two different rooms. Both are given k bits of information regarding a particular (static) scene and are asked to visualise the same in their minds. What is the minimum threshold value k beyond which the probability that they visualise different scenes drops below some small {% raw %}$$\epsilon>0$${% endraw %}"*

The idea is to observe how integration could vastly reduce the mismatch of priors while improving the latency of the benchmark. In this case:-

bits<sub>total3</sub> = {% raw %}$$[100, 200] bits$${% endraw %}

t<sub>total3</sub> - t<sub>i</sub> = {% raw %}$$[50,500]ms$${% endraw %}

Of course, these numbers mentioned above are strictly hypothetical and neuroimaging techinques are still advancing towards the goal of accurately reading noisy brain signals. 

Reference: [MinD-Vis: Seeing beyond the Brain](https://arxiv.org/pdf/2211.06956)

The notions of what neuroAI systems could do are still in its very nascent stages. In the upcoming posts, we will discuss more regarding what neuroAI systems are, what are some methods to implement the ideas discussed and how they can be applied to improve different facets of the human experience.
